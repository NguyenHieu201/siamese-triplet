...
def main(cfg: Namespace) -> None:
    ...
    train_batch_sampler = BalancedBatchSampler(train_ds.train_labels, n_classes=20, n_samples=30)
    valid_batch_sampler = BalancedBatchSampler(valid_ds.train_labels, n_classes=20, n_samples=30)
...
if __name__ == '__main__':
    ...
    train_dfs = pd.read_csv("./features.csv")
    train_dfs.name = train_dfs.name.astype("category")
    valid_dfs = pd.read_csv("./features.csv")
    valid_dfs.name = valid_dfs.name.astype("category")
